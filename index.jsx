...
function MyApp() {
	...
	useEffect(() => {
		const grassimg = new Image(); const birdimg = new Image(); const pipeimg = new Image(); const
			fpipeimg = new Image();
		...
		({ clientWidth: w, clientHeight: h } = canvasRef.current.parentElement);
		...
		interval = setInterval(() => {
			...
			for (let i = 0; i < pipes.length; i += 1) {
				if (gamestarted && !gameover) {
					...
					const mercy = 0;
					if (y < pipes[i].y - (verticalgap / hmeters * h) && x < pipes[i].x + pipew - mercy && x + birdw > pipes[i].x + mercy
						|| y + birdh >= grassh - 5
						|| y + birdh > pipes[i].y && x < pipes[i].x + pipew - mercy && x + birdw > pipes[i].x + mercy) {
						gameover = Date.now();
						vy = 2;
						deathSound.play();
					}
	...
	return (
			<div className="min-h-[100vh] bg-cover bg-bottom bg-[url('https://user-images.githubusercontent.com/18351809/46888871-624a3900-ce7f-11e8-808e-99fd90c8a3f4.png')] grow">
				<canvas tabIndex="0" onMouseDown={onMouseDown} ref={canvasRef} />
			</div>
	);
}
