...
const wingSounds = 5;
const baseWingSound = new Audio(
  "https://www.myinstants.com/media/sounds/sfx_wing.mp3"
);
const sounds = {
  wing: Array(wingSounds)
    .fill(0)
    .map((f) => baseWingSound.cloneNode()),
...
function newGame() {
  ...
  const eventListeners = [
    ...
    ([type, listener, options]) =>
      new RemovableListener(type, listener, options)
  );
  ...
  function frame() {
    ...
    if (gameStarted) {
      y =
        100 - (100 - y + vy * timeDelta + (1 / 2) * g * timeDelta * timeDelta);
    ...
    ctx.fillText(
      text,
      10 * scale,
      10 * scale + textmeasure.actualBoundingBoxAscent
    );
    ctx.fillStyle = "black";
    ctx.strokeText(
      text,
  ...
  function tap() {
    if (gameOverDate && allowRestart) {
      ...
      vy = tapBoost;
      sounds.wing[wingsoundIndex].play().catch((e) => 0);
