const log = (msg) =>
  console.log(
    `${location.href.slice(location.href.lastIndexOf("/") + 1)}: ${msg}`
  );
const delay = (del) => new Promise((res) => setTimeout(res, del));

// these things are revisioned, and are loaded for offline use
const requiredOffline = new Map(
  __MANIFEST__.map(([pathname, rev]) => [
    location.origin + pathname,
    { rev, alreadyFetching: false },
  ])
);
...
/*
downloads files needed offline into the cache, and pushes old things out
*/
async function loadAppCache() {
  ...
  alreadyInCache
    .filter((url) => !expectedCacheKeys.includes(url))
    .forEach((url) => {
      appcache.delete(url);
